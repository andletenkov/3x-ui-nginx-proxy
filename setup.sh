...
install_3xui_and_inbounds() {
  ...
  [[ -n "$PANEL_PORT" && -n "$PANEL_PATH" && -n "$XUI_USERNAME" && -n "$XUI_PASSWORD" && -n "$CLIENT_UUID" \
     && -n "$VLESS_ENCRYPTION_SERVER_KEY" && -n "$VLESS_ENCRYPTION_CLIENT_KEY" ]] ||
    die "3x-ui install/configuration did not produce PANEL_PORT/PANEL_PATH/XUI_USERNAME/XUI_PASSWORD/CLIENT_UUID/VLESS_ENCRYPTION_SERVER_KEY/VLESS_ENCRYPTION_CLIENT_KEY."
